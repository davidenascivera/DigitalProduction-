...
// Generated:	Mon Jan 29 23:46:33 2024
...
nextofFunc1(load* thisObj)
{
	...
	ind = (ind+1) % 2;
	return List1[ind].value(thisObj);
}
...
Label3: ; // Step 3
		...
		{
			thisObj->nextproc = nextofFunc1(thisObj); /* send to ... */
			EntityChanged(W_LOAD);
			retval = Continue;
			goto LabelRet;
		}
...
Label5: ; // Step 5
		...
		{
			thisObj->nextproc = &(am2_P_painting[1]); /* send to ... */
			EntityChanged(W_LOAD);
			retval = Continue;
			goto LabelRet;
		}
...
Label5: ; // Step 5
						...
						{
							am2_V_truck_isArrived = 3;
							EntityChanged(0x01000000);
						}
						{
							{
								char* pArg1 = "The production finished earlier";
...
Label2: ; // Step 2
		}
		{
			if (am2_V_truck_isArrived == 0) {
				...
				{
					am2_V_delay = (am2_V_schedule_truck - FromModelTime(ASIclock, UNITSECONDS)) / 60;
					EntityChanged(0x01000000);
				}
				{
