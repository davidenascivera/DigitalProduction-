...
newattrib(load* thisLoad, load* that)
{
	...
	if (!thisLoad->attribute) {
		...
		if (thisLoad->attribute->am_model.am_V_type)
			ResetString(&thisLoad->attribute->am_model.am_V_type);
	}
	if (that) {
		*(thisLoad->attribute) = *(that->attribute);
		if (that->attribute->am_model.am_V_type) {
			thisLoad->attribute->am_model.am_V_type = amodNULL;
			SetString(&thisLoad->attribute->am_model.am_V_type, that->attribute->am_model.am_V_type);
		}
	} else {
...
get_ldatt_address(load* thisLoad, int32 index, int32* d)
{
	...
	case 1:
		...
		break;
	case 2:
		data = (void*)&thisLoad->attribute->am_model.am_V_rugos;
		break;
	case 3:
		data = (void*)&thisLoad->attribute->am_model.am_V_start_time;
		break;
	case 4:
		data = (void*)&thisLoad->attribute->am_model.am_V_type_ID;
		break;
	case 5:
		data = (void*)&thisLoad->attribute->am_model.am_V_Random;
		break;
	}
	return data;
}
...
set_attdata(load* thisLoad)
{
	...
	am_model.am_V_rugos$att->data = (void*)&thisLoad->attribute->am_model.am_V_rugos;
	am_model.am_V_start_time$att->data = (void*)&thisLoad->attribute->am_model.am_V_start_time;
	am_model.am_V_type_ID$att->data = (void*)&thisLoad->attribute->am_model.am_V_type_ID;
	am_model.am_V_Random$att->data = (void*)&thisLoad->attribute->am_model.am_V_Random;
}
